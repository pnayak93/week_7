...
#SBATCH --cpus-per-task=10    ## number of cores the job needs, can the
...
java -jar  /opt/apps/picard-tools/1.87/AddOrReplaceReadGroups.jar I="ATACalign_""${array[j]}"".sort.bam" O="ATACalign_""${array[j]}"".RG.bam" SORT_ORDER=coordinate RGPL=illumina RGPU=D109LACXX RGLB=Lib1 RGID="${array[j]}" RGSM="${array[j]}" VALIDATION_STRINGENCY=LENIENT
